...
def calculate_5min_pct_increase_and_highest(candles_df):
    """
    計算每分鐘的移動平均線 (MA5)、累計最高價 (highest)，
    以及五分鐘內的漲幅百分比 (5min_pct_increase)。

    Parameters:
        candles_df (pd.DataFrame): 包含一分K數據的 DataFrame

    Returns:
        pd.DataFrame: 更新後的 DataFrame，包含 MA5、highest 和 5min_pct_increase 欄位
    """
    # 計算 MA5
    ...
    for idx, row in candles_df.iterrows():
        ...
        candles_df.at[idx, 'highest'] = highest

        # 取過去5分鐘的收盤價
        close_window = candles_df['close'].iloc[max(0, idx-4):idx+1]
        if len(close_window) < 5 or close_window.isnull().any():
            pct_increase = 0
        else:
            ...
            if close_window.idxmax() < close_window.idxmin():
                ...
                pct_increase = ((max_close - min_close) / min_close) * 100
        candles_df.at[idx, '5min_pct_increase'] = pct_increase

    # 對於前4筆數據，MA5會是 NaN，填補為累計平均值
    candles_df['MA5'].fillna(candles_df['close'].expanding().mean(), inplace=True)
...
def process_group_data(stock_data_collection, wait_minutes, hold_minutes, matrix_dict_analysis, verbose=True):
    ...
    while idx < total_bars:
        ...
        if in_position and not has_exited:
            ...
            elif current_time_str == '13:30:00':
                profit, profit_rate = exit_trade(
                    stock_data_collection[current_position['symbol']],
                    current_position['shares'],
                    current_position['entry_price'],
        ...
        if in_position and not has_exited:
            ...
            if not current_row.empty:
                ...
                if trigger_exit:
                    ...
                    profit_rate = (profit * 100) / (current_position['sell_cost'] - current_position['entry_fee'] - exit_fee) if (current_position['sell_cost'] - current_position['entry_fee'] - exit_fee) != 0 else 0.0
                    message_log.append(
                        (current_time_str,
                    ...
                    message_log.append(
                        (current_time_str,
                         ...
                         f"報酬率：{profit_rate:.2f}%，手續費：{exit_fee} 元{RESET}")
                    )
                    total_trades += 1
                    total_profit += profit
                    total_profit_rate += profit_rate
        ...
        for symbol in stock_symbols:
            ...
            # 確保 'limit_up_price' 欄位存在
            if 'limit_up_price' in current_row.columns:
                limit_up_price = current_row['limit_up_price'].values[0]
            else:
                limit_up_price = None  # 或者設定一個默認值
        ...
        if final_check_active:
            ...
            if leader and row.get(f'high_{leader}', None) == row.get('limit_up_price', None):
                idx += 1
                continue
    ...
    if total_trades > 0:
        ...
        print(f"\n族群 {group_name} 的模擬交易完成，總利潤：{total_profit} 元，平均報酬率：{avg_profit_rate:.2f}%\n")
...
def exit_trade(
    ...
    global transaction_fee, transaction_discount, trading_tax  # 宣告全域變數
    ...
    if pre_market_start <= current_time <= pre_market_end:
        ...
        while not stop_trading:
            ...
            if market_start <= current_time <= market_end:
                ...
                print(f"\n{datetime.now().strftime('%Y-%m-%d %H:%M:%S')} 市場開盤中，取得 {fetch_time_str} 分的即時一分K數據。")
                for symbol in symbols_to_analyze[:max_symbols_to_fetch]:
                    # 在監控循環中不計算API請求次數
                    # 不需要檢查55次限制
                    print(f"正在取得 {symbol} 的一分K數據從 {fetch_time_str}:00 到 {fetch_time_str}:00...")
                    yesterday_close = yesterday_close_prices.get(symbol, 0)
                    intraday_df = fetch_intraday_data(
                        client=client,
                        symbol=symbol,
                ...
                process_live_trading_logic(
                    symbols_to_analyze[:max_symbols_to_fetch],
                    fetch_time_str,
                    wait_minutes,
                    hold_minutes,
                    message_log,
                ...
                if current_time > market_end:
                    # 盤後處理
                    print(f"目前為 {current_time.strftime('%H:%M:%S')}，盤後時間。")
                    # （1）檢查 auto_daily 中（跳過或更新日K線數據）
                    # 已在初始數據獲取與比對階段完成
                    ...
                    for symbol in symbols_to_analyze[:max_symbols_to_fetch]:
                        if initial_api_count >= 55:
                            ...
                            initial_api_count = 0
                        # 定義要獲取的時間範圍
                        print(f"正在取得 {symbol} 的一分K數據從 09:00 到 13:30...")
                        yesterday_close = yesterday_close_prices.get(symbol, 0)
                        intraday_df = fetch_intraday_data(
                            client=client,
                            symbol=symbol,
                    ...
                    # 非盤前、盤中、盤後時間（例如深夜）
                    print(f"目前為 {current_time_str}，非盤前、盤中、盤後時間。")
                # 等待1分鐘後再檢查
                time_module.sleep(60)
...
def calculate_5min_pct_increase(new_candles, existing_candles):
    """
    根據指定的計算方式，計算每一筆新K線的「5min_pct_increase」。
    :param new_candles: 新獲取的K線數據列表（字典）。
    :param existing_candles: 已存在的K線數據列表（字典）。
    :return: 更新後的新K線數據列表（字典）。
    """
    for candle in new_candles:
        # 初始化
        candle['5min_pct_increase'] = 0.0
        ...
        if num_existing_candles == 0:
            ...
            if num_existing_candles < 4:
                ...
                relevant_candles = existing_candles[-4:] + [candle]
            ...
            # 判斷趨勢
            if close_prices.index(max_close) > close_prices.index(min_close):
                # 趨勢向上
                pct_increase = ((max_close - min_close) / min_close) * 100
            else:
                # 趨勢向下
            ...
            candle['5min_pct_increase'] = round(pct_increase, 2)
...
def process_live_trading_logic(
    ...
    for symbol in symbols_to_analyze:
        if symbol in auto_intraday_data:
            ...
            stock_data_collection[symbol]['time'] = pd.to_datetime(stock_data_collection[symbol]['time'], format='%H:%M:%S').dt.time
        else:
            stock_data_collection[symbol] = pd.DataFrame()

    # 檢查是否已經過了 13:30
    if trading_time >= time(13, 30):
        if in_position and not has_exited:
            # 強制出場
            selected_stock_df = stock_data_collection[current_position['symbol']]
            exit_price = selected_stock_df[selected_stock_df['time'] == trading_time]['close'].values[0]
            profit, return_rate = exit_trade_live(
                selected_stock_df,
                current_position['shares'],
                current_position['entry_price'],
                current_position['sell_cost'],
    ...
    # 更新 tracking_stocks 和 previous_rise_values
    for symbol in symbols_to_analyze:
        if symbol in stock_data_collection and not stock_data_collection[symbol].empty:
            current_row = stock_data_collection[symbol][stock_data_collection[symbol]['time'] == trading_time]
            if not current_row.empty:
                rise = current_row['rise'].values[0]
                previous_rise_values[symbol] = rise
    ...
    if in_position and not has_exited:
        ...
        selected_stock_df = stock_data_collection[symbol]

        current_row = selected_stock_df[selected_stock_df['time'] == trading_time]
        if not current_row.empty:
            current_high = current_row['high'].values[0]
            stop_loss_threshold = current_position['stop_loss_threshold']
            stop_loss_type = current_position['stop_loss_type']
            ...
            if trigger_exit:
                ...
                if hold_minutes is not None and hold_time >= hold_minutes:
                    exit_price = current_row['close'].values[0]
                    exit_reason = "持有時間達到"
                    profit, return_rate = exit_trade_live(
                        selected_stock_df,
                        current_position['shares'],
            ...
            print(f"無法取得 {symbol} 在 {current_time_str} 的數據，無法進行交易判斷。")
        ...
        if can_trade:
            ...
            for symbol in symbols_to_analyze:
                ...
                if five_min_increase >= 1.5:
                    any_condition_one_triggered = True
                    leader = symbol
                    print(f"{symbol} 觸發條件一，開始監測五分鐘漲幅，五分鐘漲幅: {five_min_increase:.2f}%")
                    break  # 只需要找到一個觸發條件一的股票

            if any_condition_one_triggered:
                in_waiting_period = True
                waiting_time = 0
                can_trade = False
...
def update_intraday_json(symbol, latest_candle):
    """
    更新 auto_intraday.json 檔案，將最新的 candle 數據加入指定的 symbol，並計算 '5min_pct_increase'。
    """
    ...
    # 計算 '5min_pct_increase' 使用現有的 candles
    calculated_candle = calculate_5min_pct_increase(latest_candle, existing_candles)
